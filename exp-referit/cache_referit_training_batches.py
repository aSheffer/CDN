...
vocab_save = './data/metadata/vocab.json'
...
def main():
	...
	for k in query_dict.keys():
	    ...
	    for q in query_dict[k]:
	        clean = ''.join([ch.lower() for ch in q.strip() if ch not in exclude]) # clean punctuation
	        words += [word for word in clean.split()]
	        tmp.append(clean)
	    query_dict[k] = tmp   
	    
	vocab = {w:i+1 for i, w in enumerate(list(set(words)))}
	vocab['<pad>']=0
	vocab['<unk>']=len(vocab)
