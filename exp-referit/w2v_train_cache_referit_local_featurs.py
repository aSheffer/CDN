...
trn_data_save = './data/training/w2v_train_data.bin'
tst_data_save = './data/training/w2v_test_data.bin'
...
def main():
	...
	for imcrop_name, des in query_dict.items():
	    ...
	    if imcrop_spatial_dict[imcrop_name]=='':
	        imcrop_spatial_dict[imcrop_name] = [np.load(trn_cache_local_features_dir + imcrop_name + '.png' + '_fc7.npy'), 
	    ...
	    for q in des:
	        ...
	        for bname in imcrop_dict[imname]:
	            if bname!=imcrop_name:
	                ...
	                if imcrop_spatial_dict[bname]=='':
	                    imcrop_spatial_dict[bname] = [np.load(trn_cache_local_features_dir + bname + '.png' + '_fc7.npy'), 
	        ...
	        if len(train_tuple_tmp)>0:
	            ...
	            np.random.shuffle(train_tuple_tmp)
	            tg_idx = np.random.choice(range(len(train_tuple_tmp)))
	...
	for imcrop_name, des in query_dict.items():
	    ...
	    for q in des:
	        ...
	        if(len(tst_tuple_tmp)>0):
	            ...
	            np.random.shuffle(tst_tuple_tmp)
	            tg_idx = np.random.choice(range(len(tst_tuple_tmp)))
