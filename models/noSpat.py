...
class Model():
	...
	'''
	This implementation for SG model is meant to test its performances without spatial information.
    '''
    ...
    def __init__(self,
        ...
        assert useCDN!=useBnorm, 'Parameters useCDN and useBnorm must have different values'
        ...
        with tf.variable_scope('rnn'):
            if use_wordAttn:
                attn_vecs = self.IMGscale*tf.contrib.layers.layer_norm(self.img, begin_norm_axis=2)
                
            cell = ALSTM(num_units=self.num_hidden, 
                    img_attn_dim=self.img_dims,
    ...
    def CDN_attention(self, q_embed):
        ...
        Uatt = self.IMGscale*tf.contrib.layers.layer_norm(self.img, begin_norm_axis=2)
        with tf.variable_scope('CDNAttn') as scope:
            ...
            Sattn = tf.nn.dropout(
                        tf.reshape(
                            self.linear(
                                tf.reshape(Uatt, (-1, self.img_dims)), 
                                self.num_hidden, 
    ...
    def bnorm_attention(self, q_embed, Urnn_norm=True, Uatt_norm=True):
        ...
        if Uatt_norm:
            ...
            # Attention vectors with batch normalization. 
