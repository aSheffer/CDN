...
class RunModel(Model):
    ...
    def build_data(self, data, start, end):
        ...
        padded_im, padded_bbox, attn_idx = [], [], []
            ...
            padded_im.append(
                np.concatenate(
                    [data[i][j][0] for j in range(1, len(data[i])-1)] + 
                        [np.full((imlen-(len(data[i])-2), im_dim), self.vocab['<pad>'], dtype=np.float32)], 
                    axis=0)
            )
            ...
            padded_bbox.append(
                np.concatenate(
                    [data[i][j][1] for j in range(1, len(data[i])-1)] + 
                        [np.full((imlen-(len(data[i])-2),bbox_dim), self.vocab['<pad>'], dtype=np.float32)], 
                    axis=0)
            )
           
            
        return np.array(attn_idx), np.array(padded_im), np.array(padded_bbox), np.array(dist_labels)
    ...
    def ground(self, data=None, start=None, end=None, 
        ...
        with sess.as_default():
            ...
            if feed_dict is None:
                attn_idx, padded_im, padded_bbox, labels = self.build_data(
                    data, start, end)
    ...
    def accuracy(self, data=None, start=None, end=None, sess=None, 
        ...
        with sess.as_default():
            ...
            if feed_dict is None:
                ...
                attn_idx, padded_im, padded_bbox, labels = self.build_data(data, start, end)
                feed_dict = {
                        self.img:padded_im,
                        self.bboxes:padded_bbox,
                        self.attn_idx:attn_idx,
    ...
    def train(self, trn_data, tst_data, epochs_num,  start_epoch=0):
        ...
        with sess.as_default():
            ...
            for epoch in range(start_epoch, epochs_num):
                ...
                for b in range(trn_nbatch):
                    attn_idx, padded_im, padded_bbox, labels = self.build_data(trn_data, 
                                                                            b*self.batch_size, 
                                                                            (b+1)*self.batch_size)
                    ...
                    loss, scores, lr, gs,  _ = sess.run(
                        [self.loss, self.scores, self.learning_rate, self.global_step, self.optimizer], 
                        feed_dict=feed_dict
                    )
                ...
                for b in range(tst_nbatch):
                    attn_idx, padded_im, padded_bbox, labels = self.build_data(tst_data,
                                                                                b*self.batch_size, 
                                                                                (b+1)*self.batch_size)
                    feed_dict = {
                        self.img:padded_im,
